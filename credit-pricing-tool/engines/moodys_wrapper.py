...
def _core_leverage_coverage(ebitda, ebit_mn, amort_mn, interest_mn, capex_mn,
                             total_debt, net_debt, ffo, rcf, fcf) -> dict:
    """Core leverage and coverage ratios (used by 25+ methodologies)."""
    return {
...
def _margin_and_scale(revenue_mn, ebit_mn, amort_mn, ebitda, avg_assets, total_debt, fcf,
                       financials) -> dict:
    """Margin, profitability, and scale metrics."""
    return {
        ...
        "gross_margin_pct": (ebitda / revenue_mn) * 100.0 if revenue_mn else 0.0,
        "operating_margin_volatility_pct": 5.0,
        "revenue_volatility_pct": 10.0,
        ...
        "net_income_avg_assets_pct": (ebit_mn * 0.7 / avg_assets) * 100.0 if avg_assets else 0.0,
        "return_on_avg_assets_pct": (ebit_mn / avg_assets) * 100.0 if avg_assets else 0.0,
        "total_sales_usd_billion": revenue_mn / 1000.0,
...
def _tech_and_media_metrics(ebit_mn, ebitda, capex_mn, interest_mn, total_debt,
                             cash_and_securities, net_assets_for_roa) -> dict:
    """Software/tech and media sector-specific metrics."""
    return {
        "operating_income_roa_net_cash_mktsec_pct": (ebit_mn / net_assets_for_roa) * 100.0 if net_assets_for_roa else 0.0,
...
def _oil_gas_auto_metrics(financials, ebitda, ebit_mn, total_debt) -> dict:
    """Oil & Gas, integrated O&G, and auto sector-specific metrics."""
    avg_prod = financials.get("avg_daily_production_mboed", 50.0)
    pd_reserves = financials.get("pd_reserves_mmboe", 200.0)
    book_cap = total_debt + financials.get("total_equity_mn", 0.0)
    return {
        "avg_daily_production_mboed": avg_prod,
        "pd_reserves_mmboe": pd_reserves,
        "proved_reserves_mmboe": financials.get("proved_reserves_mmboe", 200.0),
        "leveraged_full_cycle_ratio_x": financials.get(
            "leveraged_full_cycle_ratio_x", (ebitda / total_debt) if total_debt else 1.0),
        "ep_debt_per_avg_daily_production_usd_per_boe_d": financials.get(
            "ep_debt_per_avg_daily_production_usd_per_boe_d",
            (total_debt * 1_000_000) / (avg_prod * 1000) if avg_prod else 0.0),
        "ep_debt_per_pd_reserves_usd_per_boe": financials.get(
            "ep_debt_per_pd_reserves_usd_per_boe",
            (total_debt * 1_000_000) / (pd_reserves * 1_000_000) if pd_reserves else 0.0),
        "crude_distillation_capacity_mbbld": financials.get("crude_distillation_capacity_mbbld", 100.0),
        "ebit_avg_book_cap_pct": (ebit_mn / book_cap) * 100.0 if book_cap else 0.0,
...
def compute_universal_metrics(financials: dict) -> dict:
    ...
    Delegates to category-specific helpers for readability.
    ...
    cash_and_securities = (financials.get("cash_mn", 0.0)
                           + financials.get("cash_like_mn", 0.0)
                           + financials.get("marketable_securities_mn", 0.0))
    ...
    # Build metrics from category helpers
    metrics = {}
    metrics.update(_core_leverage_coverage(
        ebitda, ebit_mn, amort_mn, interest_mn, capex_mn,
        total_debt, net_debt, ffo, rcf, fcf))
    metrics.update(_margin_and_scale(
        revenue_mn, ebit_mn, amort_mn, ebitda, avg_assets, total_debt, fcf, financials))
    metrics.update(_tech_and_media_metrics(
        ebit_mn, ebitda, capex_mn, interest_mn, total_debt,
        cash_and_securities, net_assets_for_roa))
    metrics.update(_oil_gas_auto_metrics(financials, ebitda, ebit_mn, total_debt))

    # Raw values for special cases in the engine
    metrics.update({
        "total_debt": total_debt, "net_debt": net_debt,
        "ebitda": ebitda, "ebit": ebit_mn,
        "rcf": rcf, "interest_expense": interest_mn,
    })
